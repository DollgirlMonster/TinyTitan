...
function home(settingsText) {
  const dshHome = mkdtempSync(join(tmpdir(), "dsh-nvmai-test-"));
...
test("the default preset is read and only set when absent", () => {
  ...
  assert.equal(setDefaultPreset(settings, "nvmai").changed, false);
  ...
  assert.equal(defaultPreset(appended.text), "nvmai");
});

test("the plugin's own preset is generated and the default is set when unset", () => {
  const dshHome = home("ui-theme:\n  preference: dark\n");
  const messages = [];
  const result = ensureCompactionPreset({ dshHome, presetId: "nvmai", standardPath: STANDARD,
                                          log: (message) => messages.push(message) });
  assert.equal(result.generated, true);
  assert.equal(result.defaultSet, true);
  const generated = readFileSync(presetAt(dshHome, "nvmai"), "utf8");
  assert.ok(generated.includes(COMPACTION_BACKEND));
  assert.equal(defaultPreset(readFileSync(join(dshHome, "settings.yaml"), "utf8")), "nvmai");
  assert.equal(backups(join(dshHome, "settings.yaml")).length, 1);
  assert.ok(messages.some((message) => message.includes("wrote the nvmai agent preset")));
});
...
test("adoption can be turned off, and a shipped default is never edited", () => {
  ...
  const result = ensureCompactionPreset({ dshHome: off, presetId: "nvmai", standardPath: STANDARD,
                                          adopt: false, log: () => {} });
  ...
  const untouched = ensureCompactionPreset({ dshHome: shipped, presetId: "nvmai",
...
test("a second run changes nothing", () => {
  const dshHome = home("agent-presets:\n  default: qwen38\n");
  writeUserPreset(dshHome, "qwen38", fixture());
  ensureCompactionPreset({ dshHome, presetId: "nvmai", standardPath: STANDARD, log: () => {} });
  const before = readFileSync(presetAt(dshHome, "qwen38"), "utf8");
  const second = ensureCompactionPreset({ dshHome, presetId: "nvmai", standardPath: STANDARD,
                                          log: () => {} });
...
test("a missing standard preset is reported, not fatal", () => {
  const dshHome = home("agent-presets:\n  default: qwen38\n");
  const messages = [];
  const result = ensureCompactionPreset({ dshHome, presetId: "nvmai",
