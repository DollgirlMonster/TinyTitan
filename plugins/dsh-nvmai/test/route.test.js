...
function repo(withScript) {
  const root = mkdtempSync(join(tmpdir(), "dsh-nvmai-route-"));
...
test("the route is refreshed by running the checkout's tool", () => {
  ...
  assert.deepEqual(calls[0].args.slice(1), [
    "--write", "--port", "8096", "--provider", "nvmai",
...
test("a checkout without the tool is reported, not fatal", () => {
  const messages = [];
  const result = registerRoute({ repoRoot: repo(false), port: 8080, provider: "nvmai",
...
test("a failed refresh is reported with the tool's own words", () => {
  ...
  const result = registerRoute({ repoRoot: repo(true), port: 8080, provider: "nvmai",
...
test("config defaults suit a local server and can be overridden", () => {
  ...
  assert.equal(defaults.provider, "nvmai");
  assert.equal(defaults.presetId, "nvmai");
  ...
  const configured = resolveConfig({ port: 8096, provider: "local", presetId: "nvmai-thin",
  ...
  assert.equal(configured.presetId, "nvmai-thin");
...
test("the checkout is found from a hint, the environment, the module, or the profile", () => {
  const base = mkdtempSync(join(tmpdir(), "dsh-nvmai-find-"));
  ...
  assert.deepEqual(findRepoRoot({ env: { NVMAI_REPO: checkout }, moduleUrl: "file:///nowhere/x.js",
                                  cwd: base }), { root: checkout, found: true });
  // Walking up from the module (running out of the checkout).
  const moduleUrl = pathToFileURL(join(checkout, "plugins", "dsh-nvmai", "src", "config.js")).href;
  ...
  const installed = join(profile, "node_modules", "dsh-nvmai", "src");
  mkdirSync(installed, { recursive: true });
  writeFileSync(join(profile, "package.json"), JSON.stringify({
    dependencies: { "dsh-nvmai": `file:${join(checkout, "plugins", "dsh-nvmai")}` },
  }));
  assert.deepEqual(
  ...
  const empty = mkdtempSync(join(tmpdir(), "dsh-nvmai-empty-"));
