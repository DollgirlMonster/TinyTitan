...
running models from a local **NVMAI** server. It is deliberately thin: the
...
2. **Mounts a compaction backend that does not think.** Compaction and session
   ...
   preset points that row at `dsh-nvmai/backend`, which forces thinking off for
   those calls only — ordinary turns keep the route's level.

## Install

```sh
dsh plugin --profile web add file:/path/to/NVMAI/plugins/dsh-nvmai
```
...
| `~/.dsh/.agent-presets/nvmai/agent.cordis.yml` | generated from the shipped `standard` preset, with the compaction row on this backend |
| `~/.dsh/settings.yaml` | `agent-presets.default: nvmai` — **only** when the file names no default |
...
every file you own untouched — then the plugin only writes its own `nvmai`
...
it (`dsh plugin --profile web remove dsh-nvmai` then `add` again) or DSH keeps
running the copy it made.

## Configure

Every field is optional; these are the defaults the `cordis.patch.yml` row writes
out, and `NVMAI_PORT` / `NVMAI_REPO` / `DSH_HOME` are the environment fallbacks.

| Field | Default | Meaning |
|---|---|---|
| `port` | `8080` | the port the NVMAI server serves on |
| `provider` | `nvmai` | the `llm-pi-ai` provider route name |
| `presetId` | `nvmai` | the agent preset this plugin generates |
...
- **No budgets, no vision, no dialects.** NVMAI accepts `reasoning_budget_tokens`
...
```sh
dsh plugin --profile web remove dsh-nvmai
rm -rf ~/.dsh/.agent-presets/nvmai
```

Then re-point the preset you keep at `@deepseek-ai/dsh-compaction-basic` (a
`.bak-*` file beside it has the row as it was), and drop
`agent-presets.default: nvmai` from `~/.dsh/settings.yaml` if this plugin set it.

## Test

```sh
cd plugins/dsh-nvmai && node --test test/
```
...
the route call against a stubbed runner. The `dsh-nvmai/backend` import itself is
