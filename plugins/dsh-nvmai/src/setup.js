...
export const COMPACTION_BACKEND = "dsh-nvmai/backend";
...
export function repointCompactionRow(
  ...
  while (index < lines.length) {
    ...
    out.push(`${indent}# Mounted by dsh-nvmai: thinking off for compaction calls.`);
...
export function ensureCompactionPreset({
  ...
  if (!existsSync(standardPath)) {
    log(`dsh-nvmai: no standard preset at ${standardPath}; not writing a compaction preset`);
    return result;
  }
  ...
  if (!existsSync(presetPath) || readFileSync(presetPath, "utf8") !== generated.text) {
    ...
    log(`dsh-nvmai: wrote the ${presetId} agent preset (${presetPath})`);
  }
  result.preset = presetId;

  const settingsPath = join(dshHome, "settings.yaml");
  if (!existsSync(settingsPath)) {
    log(`dsh-nvmai: no ${settingsPath}; select the ${presetId} preset yourself`);
    return result;
  }
  ...
  if (active === null && setDefault) {
    ...
    if (merged.changed) {
      ...
      log(`dsh-nvmai: set agent-presets.default to ${presetId}`);
    }
    return result;
  }
  if (active === presetId) return result;
  if (active === null) {
    log(`dsh-nvmai: no default agent preset; select ${presetId} to mount the backend`);
    return result;
  }
  if (!adopt) {
    log(`dsh-nvmai: the default agent preset is ${active}; ${presetId} has the quiet backend`);
    return result;
  }
  const activePath = join(dshHome, ".agent-presets", active, "agent.cordis.yml");
  if (!existsSync(activePath)) {
    log(`dsh-nvmai: the default agent preset ${active} is not a user preset; select ${presetId} to mount the backend`);
    return result;
  }
  ...
  result.adopted = active;
  log(`dsh-nvmai: pointed the ${active} preset's compaction row at the quiet backend`);
  return result;
}
