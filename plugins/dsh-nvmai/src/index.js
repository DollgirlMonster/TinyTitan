/**
 * `dsh-nvmai` — the DeepSeek Harness side of running models from a local NVMAI
 * server.
 *
 ...
 *    plugin generates points that row at `dsh-nvmai/backend`.
 *
...
export function apply(ctx, config = {}) {
  ...
  if (resolved.registerRoute) {
    try {
      ...
      log(`dsh-nvmai: route registration threw: ${error instanceof Error ? error.message : error}`);
    }
  }
  if (resolved.writeCompactionPreset) {
    try {
      ...
      log(`dsh-nvmai: compaction preset threw: ${error instanceof Error ? error.message : error}`);
    }
  }
}
