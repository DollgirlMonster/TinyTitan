/**
 * Keep the harness's route to NVMAI current, by asking the checkout.
 *
...
export function registerRoute({
  ...
  if (!existsSync(script)) {
    const why = repoFound === false
      ? "no NVMAI checkout found (set NVMAI_REPO or the repoRoot config)"
      : `${script} does not exist`;
    log(`dsh-nvmai: ${why}; leaving the llm-pi-ai route as it is`);
  ...
  try {
    ...
    log(`dsh-nvmai: route refreshed from ${script} (${first})`);
    return { status: "written", script, detail: first };
  } catch (error) {
    const detail = String(error?.stderr ?? error?.message ?? error).trim().split("\n")[0];
    log(`dsh-nvmai: route refresh failed: ${detail}`);
