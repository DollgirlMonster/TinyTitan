...
enum SyntheticSnapshot {
    ...
    /// tests can pre-compute byte-fidelity expectations. TinyTitan is Qwen-only,
    ...
    static func buildQwen(
        at dir: String,
        weightBits: Int = 4,
        seed: UInt64 = 0xC0FF_EE00_9A11_AB1E
    ) throws -> Snapshot {
        ...
        appendUnquantizedBF16(
            name: "vision_tower.blocks.0.norm1.weight",
            shape: [arch.hidden], into: &tensors, rng: &rng)
        appendUnquantizedBF16(
        ...
        try configData.write(
            to: URL(fileURLWithPath: (dir as NSString).appendingPathComponent("config.json")))
    ...
    static func buildQwenMTP(
        at dir: String,
        weightBits: Int = 4,
        seed: UInt64 = 0x0A11_CE55_1DE0_0001
    ) throws -> Snapshot {
        precondition([4, 8].contains(weightBits))
        try? FileManager.default.removeItem(atPath: dir)
        try FileManager.default.createDirectory(
        ...
        for projection in ["k_proj", "v_proj"] {
            appendQuantizedWeight(
                name: prefix + ".self_attn." + projection,
                outerShape: [arch.numKVHeads * arch.headDim],
                innerLogical: arch.hidden, bits: weightBits,
                groupSize: arch.groupSize, into: &tensors, rng: &rng)
        }
        ...
        for role in ["gate_proj", "up_proj"] {
            appendQuantizedWeight(
                name: prefix + ".mlp.shared_expert." + role,
        ...
        appendUnquantizedBF16(
            name: "norm.weight", shape: [arch.hidden],
            into: &tensors, rng: &rng)
        ...
        let weightMap = Dictionary(
            uniqueKeysWithValues: tensors.map {
                ($0.0, shardName)
            })
        ...
        try JSONSerialization.data(withJSONObject: index, options: [.sortedKeys])
            .write(
                to: URL(
                    fileURLWithPath: (dir as NSString).appendingPathComponent(
    ...
    private static func appendQuantizedWeight(
        name: String,
        outerShape: [Int],
        innerLogical: Int,
        bits: Int,
    ...
    private static func appendUnquantizedBF16(
    ...
    private static func writeShard(
        path: String,
        tensors: [(String, String, [Int], [UInt8])]
    ) throws {
        var off: UInt64 = 0
