...
@Suite struct PrefillMoETests {
    @Test func tokenMajorReduceMatchesCPUReference() throws {
        ...
        let expected = Self.cpuReduce(
            routePartials: partials,
        ...
        try prefill.encodeReduceTokenMajor(
            commandBuffer: cb,
    ...
    private static func cpuReduce(
        routePartials: [Float16],
        ...
        for token in 0..<rows {
            for column in 0..<d {
                ...
                for rank in 0..<topK {
                    ...
                    accumulator +=
                        Float(routeWeights[token * topK + rank])
