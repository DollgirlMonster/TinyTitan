...
@Suite struct PrefillQKVEpilogueTests {
    @Test func blockQKVEpilogueMatchesRepeatedScalarSWA() throws {
        _ = try Self.expectMatchesRepeatedScalar(
            rows: 3,
            numQHeads: 16,
            numKVHeads: 8,
            headDim: 256,
    ...
    @Test func blockQKVEpiloguePreservesFullLayerSharedRawKVQuirk() throws {
        let result = try Self.expectMatchesRepeatedScalar(
            rows: 2,
            numQHeads: 16,
            numKVHeads: 2,
            headDim: 512,
            ...
            seed: 0x7A50_F011)
        #expect(
            result.kUsed != result.vUsed,
            "full-layer K/V must diverge after K norm+RoPE and V no-scale norm")
    }

    private static func expectMatchesRepeatedScalar(
        rows: Int,
        numQHeads: Int,
        numKVHeads: Int,
        headDim: Int,
        ...
        guard
            let qWeightBuf = ctx.device.makeBuffer(
                bytes: qWeight,
                length: qWeight.count * MemoryLayout<UInt16>.size,
                options: .storageModeShared),
            let kWeightBuf = ctx.device.makeBuffer(
                bytes: kWeight,
        ...
        for row in 0..<rows {
            ...
            try scalarNorm.encodeBF16WPerHead(
                commandBuffer: cb,
                x: qRef,
                xOffset: qOffset,
                weight: qWeightBuf,
            ...
            try scalarNorm.encodeBF16WPerHead(
                ...
                numHeads: numKVHeads,
                eps: 1e-6)
            try scalarNorm.encodeNoScalePerHead(
                commandBuffer: cb,
                x: vRef,
                xOffset: kvOffset,
                out: vRef,
            ...
            if rotatedPairs * 2 == UInt32(headDim) {
                try scalarRoPE.encodeDefaultNeox(
                    commandBuffer: cb,
                    data: qRef,
                    dataOffset: qOffset,
                ...
                try scalarRoPE.encodeProportionalNeox(
                    commandBuffer: cb,
                    data: qRef,
                    dataOffset: qOffset,
                    position: UInt32(startPosition + row),
        ...
        Self.expectClose(
            Fp16Buffer.read(qBlock, count: rows * qStride),
            Fp16Buffer.read(qRef, count: rows * qStride),
            label: "Q")
        Self.expectClose(
            Fp16Buffer.read(kBlock, count: rows * kvStride),
            Fp16Buffer.read(kRef, count: rows * kvStride),
            label: "K")
        Self.expectClose(
            Fp16Buffer.read(vBlock, count: rows * kvStride),
        ...
        return (
            Self.usedRows(kActual, rows: rows, rowStride: kvStride, usedPerRow: kvUsed),
    ...
    private static func assertPaddingUnchanged(
        _ buffer: MTLBuffer,
        original: [Float16],
        rows: Int,
        rowStride: Int,
        ...
        for row in 0..<rows {
            for i in usedPerRow..<rowStride {
                #expect(
                    ptr[row * rowStride + i] == original[row * rowStride + i],
