...
extension PrefillGroupedRoutedMoETests {
    @Test func streamedParamsLayoutIsStable() {
        ...
        #expect(
            MemoryLayout.offset(of: \PrefillGroupedRoutedMoEStreamedParams.pairStart) == .some(0))
        #expect(
            MemoryLayout.offset(of: \PrefillGroupedRoutedMoEStreamedParams.pairCount) == .some(4))
        ...
        #expect(
            MemoryLayout.offset(of: \PrefillGroupedRoutedMoEStreamedParams.hiddenStrideElements)
                == .some(20))
        #expect(
            MemoryLayout.offset(of: \PrefillGroupedRoutedMoEStreamedParams.liveExpertCount)
                == .some(24))
        #expect(
            MemoryLayout.offset(of: \PrefillGroupedRoutedMoEStreamedParams.localExpert0)
                == .some(28))
        #expect(
            MemoryLayout.offset(of: \PrefillGroupedRoutedMoEStreamedParams.localExpert15)
                == .some(88))
        #expect(
            MemoryLayout.offset(of: \PrefillGroupedRoutedMoEStreamedParams.gateWOff) == .some(92))
        #expect(
            MemoryLayout.offset(of: \PrefillGroupedRoutedMoEStreamedParams.downBOff) == .some(124))
    }
