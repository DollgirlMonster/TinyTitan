...
@Suite struct PrefillRouterTests {
    ...
    @Test func blockRouterMatchesRepeatedScalarRows() throws {
        ...
        for row in 0..<rows {
            ...
            let (idx, wt) = try Self.runScalarRouter(
                ctx: ctx,
                moe: moe,
                buffers: buffers,
    ...
    @Test func blockRouterNearTieMatchesScalarPath() throws {
        ...
        let buffers = try Self.makeBuffers(
            ...
            perExpertScale: perExpertScale,
            rows: 1)
        let (scalar, _) = try Self.runScalarRouter(
            ctx: ctx,
            moe: moe,
            buffers: buffers,
            hiddenRow: hidden)
    ...
    private static func makeBuffers(
        ...
        guard
            ...
            let hBuf = ctx.device.makeBuffer(
                ...
                options: .storageModeShared),
            let eBuf = ctx.device.makeBuffer(
                bytes: effBits,
                length: effBits.count * MemoryLayout<UInt16>.size,
                options: .storageModeShared),
            let pBuf = ctx.device.makeBuffer(
                bytes: pesBits,
                length: pesBits.count * MemoryLayout<UInt16>.size,
                options: .storageModeShared),
            let idxBuf = ctx.device.makeBuffer(
                length: rows * Self.topK * MemoryLayout<UInt32>.size,
                options: .storageModeShared),
            let wtBuf = ctx.device.makeBuffer(
                length: rows * Self.topK * MemoryLayout<Float16>.size,
                options: .storageModeShared)
        else {
    ...
    private static func runScalarRouter(
        ctx: MetalContext,
        moe: MoE,
        buffers: RouterBuffers,
        hiddenRow: [Float16]
    ) throws -> ([UInt32], [Float16]) {
        guard
            let hBuf = ctx.device.makeBuffer(
                bytes: hiddenRow,
        ...
        return (
            readUInt32(idxBuf, count: Self.topK),
    ...
    private static func makeNearTieWeights() -> [[Float]] {
        var weights = [[Float]](
            repeating: [Float](repeating: 0, count: Self.d),
            count: Self.experts)
    ...
    private static func makeHiddenBlock(
        rows: Int,
        rowStride: Int,
        d: Int,
    ...
    private static func readUInt32(_ buffer: MTLBuffer, count: Int) -> [UInt32] {
        (0..<count).map {
            buffer.contents().load(
                fromByteOffset: $0 * MemoryLayout<UInt32>.size,
                as: UInt32.self)
        }
    }

    private static func assertWeightsClose(
        _ got: [Float16],
        _ expected: [Float16],
        tolerance: Float
    ) {
