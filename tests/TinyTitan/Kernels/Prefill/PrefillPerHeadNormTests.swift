...
@Suite struct PrefillPerHeadNormTests {
    ...
    private static func runBF16Case(
        rows: Int,
        heads: Int,
        headDim: Int,
        seed: UInt64,
        ...
        #expect(
            actual == reference,
            "block BF16 per-head norm must match repeated scalar per-token path for \(label)")
        assertPaddingUnchanged(
            candidate,
            original: input,
            rows: rows,
            rowStride: rowStride,
    ...
    private static func runNoScaleCase(
        rows: Int,
        heads: Int,
        headDim: Int,
        seed: UInt64,
        ...
        let cb = try #require(ctx.queue.makeCommandBuffer())
        for row in 0..<rows {
            try scalar.encodeNoScalePerHead(
                commandBuffer: cb,
                x: ref,
                xOffset: row * rowStride * MemoryLayout<Float16>.size,
                out: ref,
        ...
        #expect(
            actual == reference,
            "block no-scale per-head norm must match repeated scalar per-token path for \(label)")
        assertPaddingUnchanged(
            candidate,
            original: input,
            rows: rows,
