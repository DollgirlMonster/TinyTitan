...
@Suite struct FusedQKVEpilogueTests {
    ...
    private static func expectMatchesLegacy(
        numQHeads: Int,
        numKVHeads: Int,
        headDim: Int,
        position: Int,
        ...
        guard
            let qLegacy = ctx.device.makeBuffer(
                bytes: q, length: Self.bytes(qCount), options: .storageModeShared),
            let kLegacy = ctx.device.makeBuffer(
                bytes: k, length: Self.bytes(kvCount), options: .storageModeShared),
            let vLegacy = ctx.device.makeBuffer(
                bytes: v,
                length: Self.bytes(kvCount),
                options: .storageModeShared),
            let qFused = ctx.device.makeBuffer(
                bytes: q, length: Self.bytes(qCount), options: .storageModeShared),
            let kFused = ctx.device.makeBuffer(
                bytes: k, length: Self.bytes(kvCount), options: .storageModeShared),
            let vFused = ctx.device.makeBuffer(
                bytes: v, length: Self.bytes(kvCount), options: .storageModeShared),
            let qW = ctx.device.makeBuffer(
                bytes: qWeight,
                length: qWeight.count * MemoryLayout<UInt16>.size,
                options: .storageModeShared),
            let kW = ctx.device.makeBuffer(
                bytes: kWeight,
        ...
        #expect(
            Self.bufferBytes(qLegacy, count: qCount) == Self.bufferBytes(qFused, count: qCount),
            "Q mismatch for headDim=\(headDim) kvHeads=\(numKVHeads)")
        #expect(
            Self.bufferBytes(kLegacy, count: kvCount) == Self.bufferBytes(kFused, count: kvCount),
            "K mismatch for headDim=\(headDim) kvHeads=\(numKVHeads)")
        #expect(
            Self.bufferBytes(vLegacy, count: kvCount) == Self.bufferBytes(vFused, count: kvCount),
