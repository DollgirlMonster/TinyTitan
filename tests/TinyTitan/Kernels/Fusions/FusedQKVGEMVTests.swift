...
@Suite struct FusedQKVGEMVTests {
    ...
    @Test func fusedQKVGEMV_matchesThreeGEMVs_offsetWeights() throws {
        try Self.expectMatches(
            qRows: 128, kvRows: 64, n: 2816, seed: 0x5156_4746,
            weightOffset: 2)
    }
    ...
    private static func expectMatches(
        ...
        guard
            let qW = ctx.device.makeBuffer(
                bytes: q.weights, length: q.weights.count,
                options: .storageModeShared),
            let qS = ctx.device.makeBuffer(
                bytes: q.scales,
                length: q.scales.count * MemoryLayout<UInt16>.size,
                options: .storageModeShared),
            let qB = ctx.device.makeBuffer(
                bytes: q.biases,
                length: q.biases.count * MemoryLayout<UInt16>.size,
                options: .storageModeShared),
            let kW = ctx.device.makeBuffer(
                bytes: k.weights, length: k.weights.count,
                options: .storageModeShared),
            let kS = ctx.device.makeBuffer(
                bytes: k.scales,
                length: k.scales.count * MemoryLayout<UInt16>.size,
                options: .storageModeShared),
            let kB = ctx.device.makeBuffer(
                bytes: k.biases,
                length: k.biases.count * MemoryLayout<UInt16>.size,
                options: .storageModeShared),
            let vW = ctx.device.makeBuffer(
                bytes: v.weights, length: v.weights.count,
                options: .storageModeShared),
            let vS = ctx.device.makeBuffer(
                bytes: v.scales,
                length: v.scales.count * MemoryLayout<UInt16>.size,
                options: .storageModeShared),
            let vB = ctx.device.makeBuffer(
                bytes: v.biases,
    ...
    private static func makeProjection(
        ...
        for row in 0..<rows {
            ...
            for i in 0..<packedPerRow {
                weights[weightOffset + row * packedPerRow + i] = q.packed[i]
            }
            for i in 0..<groups {
