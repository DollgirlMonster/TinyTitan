...
/// Spawns the real `TinyTitanDecodeService` binary and drives it over its Unix
...
@Suite struct DecodeServiceProcessTests {
    ...
    @Test func processHandshakeFailedLoadAndShutdown() throws {
        ...
        let socketDirectory = URL(fileURLWithPath: "/tmp/tinytitan-\(getuid())", isDirectory: true)
    ...
    /// lives under `.build/<triple-or-flat>/debug/TinyTitanDecodeService`.
...
private enum DecodeServiceProcessError: Error, CustomStringConvertible {
    ...
    var description: String {
        switch self {
        case .binaryNotFound:
            "TinyTitanDecodeService debug binary not found; run `swift build --build-tests` first"
        case .timedOut:
