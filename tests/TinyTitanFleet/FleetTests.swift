...
@Suite struct FleetTargetTests {
    @Test func parsesHostPortAndIPv6() throws {
        #expect(FleetTarget(text: "192.168.18.25", defaultPort: 3080) == FleetTarget(host: "192.168.18.25", port: 3080))
        #expect(FleetTarget(text: "node3.local:3081", defaultPort: 3080)?.port == 3081)
        #expect(FleetTarget(text: "[fd7a:115c:a1e0::5]:3080", defaultPort: 3080)?.host == "fd7a:115c:a1e0::5")
        #expect(FleetTarget(text: "100.114.69.128:3080", defaultPort: 3080)?.host == "100.114.69.128")
    }
...
@Suite struct FleetRoutingTests {
    @Test func aPromptGoesToTheOwnerAndNeverToTheSeed() async throws {
        let (runner, transport, group) = try await loadedGroup(
            replies: ["/dsh-lan/prompt": FakeTransport.Reply(status: 200, body: Data(#"{"ok":true}"#.utf8))]
        )
        let outcome = try await runner.prompt(group: group, sessionId: "s-remote", text: "checkpoint")
        #expect(outcome.ok)
        let calls = await transport.actions()
        #expect(calls == ["POST 100.114.69.128:3080/dsh-lan/prompt"],
    ...
    @Test func promptAllReportsEverySessionAndSurvivesAFailure() async throws {
        let (runner, transport, group) = try await loadedGroup(
            replies: ["/dsh-lan/prompt": FakeTransport.Reply(
                status: 404, body: Data(#"{"error":"not-found","message":"session has no live agent"}"#.utf8))]
        )
        let outcomes = await runner.promptAll(group: group, text: "status?", concurrency: 2)
        #expect(outcomes.count == 2, "every active session is attempted, including the one that fails")
    ...
    @Test func mutatingPathsAreEncodedAndTargeted() async throws {
        ...
        let (runner, transport, group) = try await loadedGroup(
            replies: [
                "/dsh-lan/sessions/s-local/archive": FakeTransport.Reply(status: 200, body: Data(#"{"ok":true}"#.utf8)),
                "/dsh-lan/workspaces/w-remote/delete": FakeTransport.Reply(status: 200, body: Data(#"{"ok":true,"deleted":true}"#.utf8)),
            ]
        )
        _ = try await runner.archive(group: group, sessionId: "s-local")
        _ = try await runner.deleteWorkspace(group: group, workspaceId: "w-remote", archiveSessions: false)
        let calls = await transport.actions()
        #expect(calls.contains("POST 127.0.0.1:3080/dsh-lan/sessions/s-local/archive"),
                "a local session is archived on the member that holds it")
        #expect(calls.contains("POST 100.114.69.128:3080/dsh-lan/workspaces/w-remote/delete"),
                "the workspace is deleted on the member that holds it")
        let body = json(await transport.actionBodyText(of: 1))
        #expect(body["archiveSessions"] as? Bool == false, "--keep-sessions is passed through, not assumed")
    }

    @Test func createWorkspaceGoesToTheNamedMember() async throws {
        let (runner, transport, group) = try await loadedGroup(
            replies: ["/dsh-lan/workspaces": FakeTransport.Reply(status: 200, body: Data(#"{"ok":true,"workspaceId":"ws-new"}"#.utf8))]
        )
        let ack = try await runner.createWorkspace(group: group, node: "Node3", path: "/Users/node3/New", title: "New")
        #expect(ack.ok)
    ...
    @Test func anHTTPFailureCarriesTheMembersOwnMessage() async throws {
        let transport = FakeTransport(replies: ["/dsh-lan/inventory": FakeTransport.Reply(
            status: 401, body: Data(#"{"error":"unauthorized","hint":"send the shared token"}"#.utf8))])
        let runner = FleetRunner(client: FleetClient(token: "wrong", transport: transport))
        let seed = try #require(FleetTarget(text: "127.0.0.1:3080", defaultPort: 3080))
        await #expect(throws: FleetError.http(target: "127.0.0.1:3080", status: 401, message: "unauthorized")) {
