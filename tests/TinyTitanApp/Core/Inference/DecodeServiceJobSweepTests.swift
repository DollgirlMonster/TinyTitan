...
@Suite struct DecodeServiceJobSweepTests {
    ...
    @Test func parsesOurJobsAndIgnoresEverythingElse() {
        let output = list(
            "-\t0\tcom.apple.Finder",
            "1234\t0\tcom.tinytitan.decode.501.98765.a1b2c3d4",
            "-\t0\tcom.tinytitan.decode.501.98766.deadbeef",
            "1234\t0\tcom.tinytitan.decode.999.11111.cafebabe",
            "1234\t0\tcom.tinytitan.decode.501.notapid.a1b2c3d4",
            "1234\t0\tcom.tinytitan.decode.501.11111.",
            "not a launchctl row at all")
        let jobs = DecodeServiceJobSweep.parseLaunchctlList(output, uid: uid)
        #expect(jobs.map(\.label) == [
            "com.tinytitan.decode.501.98765.a1b2c3d4",
            "com.tinytitan.decode.501.98766.deadbeef",
        ])
    ...
    @Test func socketNameMatchesTheLaunchPath() {
        let label = "com.tinytitan.decode.501.4321.feedface"
    ...
    @Test func onlyJobsWithADeadOwnerAreOrphans() {
        ...
        #expect(orphans.map(\.label) == ["com.tinytitan.decode.501.98766.deadbeef"])
    }
    ...
    @Test func aliveOwnerIsLeftAloneEvenWhenTheHelperIsNotRunning() {
        let output = list("-\t0\tcom.tinytitan.decode.501.22222.0badc0de")
        let jobs = DecodeServiceJobSweep.parseLaunchctlList(output, uid: uid)
