...
final class SharedExpertAffineQuant {
    ...
    func encode(
        ...
        guard scratchGateOffset + bytes <= scratchGate.length,
            ...
            scratchActOffset + bytes <= scratchAct.length
        else {
            throw SharedExpertError.scratchTooSmall("need \(bytes) bytes per intermediate buffer")
        }
        func project(
            _ p: SharedExpertProjection, _ input: MTLBuffer,
        ...
        encoder.dispatchThreads(
            ...
            threadsPerThreadgroup: MTLSize(
                width: min(256, activationPSO.maxTotalThreadsPerThreadgroup), height: 1, depth: 1))
        encoder.endEncoding()
