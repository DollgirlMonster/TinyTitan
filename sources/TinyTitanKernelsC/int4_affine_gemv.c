...
#define TINYTITAN_MAX_GROUPS 256

static inline float tinytitan_bf16(uint16_t bits) {
...
void tinytitan_int4_affine_gemv(const uint8_t *weights,
    ...
    float group_xsum[TINYTITAN_MAX_GROUPS];
    const size_t xsum_groups = groups <= TINYTITAN_MAX_GROUPS ? groups : 0;
    ...
    for (size_t r = 0; r < rows; ++r) {
        ...
        for (size_t g = 0; g < groups; ++g) {
            const uint8_t *wg = w_row + g * (TINYTITAN_GROUP_SIZE / 2);
            const float *xg = x + g * TINYTITAN_GROUP_SIZE;
            ...
            for (size_t k = 0; k < TINYTITAN_GROUP_SIZE / 2; k += 16) {
            ...
            acc += tinytitan_bf16(s_row[g]) * vaddvq_f32(dot)
                 + tinytitan_bf16(b_row[g]) * xsum;
        }
        out[r] = acc;
    }
#else
    ...
    for (size_t r = 0; r < rows; ++r) {
        ...
        for (size_t g = 0; g < groups; ++g) {
            ...
            for (size_t k = 0; k < TINYTITAN_GROUP_SIZE / 2; ++k) {
                const float x0 = xg[k * 2], x1 = xg[k * 2 + 1];
                dot += (float)(wg[k] & 0x0F) * x0 + (float)(wg[k] >> 4) * x1;
                xsum += x0 + x1;
            }
            acc += tinytitan_bf16(scales[r * groups + g]) * dot
                 + tinytitan_bf16(biases[r * groups + g]) * xsum;
        }
        out[r] = acc;
    }
#endif
