    ...
    static func validate(path: String, plan: RepackPlan) throws {
        ...
        guard let root = try JSONSerialization.jsonObject(with: data) as? [String: Any],
            ...
            throw RepackError.configurationInvalid(
                detail: "layout.json validation failed: malformed root")
        }
        for layerObj in layers {
            guard let layerIndex = layerObj["layer"] as? Int,
                let experts = layerObj["experts"] as? [[String: Any]],
                let planLayer = plan.layers.first(where: { $0.layerIndex == layerIndex })
            else {
                throw RepackError.configurationInvalid(
                    detail: "layout.json validation failed: malformed layer")
            }
            ...
            for expertObj in experts {
                guard let expert = expertObj["expert"] as? Int,
                    ...
                    throw RepackError.configurationInvalid(
                        detail: "layout.json validation failed: malformed expert")
                }
                guard expert >= 0 && expert < planLayer.expertsPerLayer else {
                    throw RepackError.configurationInvalid(
                        detail: "layout.json validation failed: expert out of range")
                }
                guard seenLogical.insert(expert).inserted else {
                    throw RepackError.configurationInvalid(
                        detail: "layout.json validation failed: duplicate expert \(expert)")
                }
                guard seenOffsets.insert(offset).inserted else {
                    throw RepackError.configurationInvalid(
                        detail: "layout.json validation failed: duplicate offset \(offset)")
                }
                guard offset % Layout.pageBytes == 0 else {
                    throw RepackError.configurationInvalid(
                        detail: "layout.json validation failed: unaligned offset \(offset)")
                }
                ...
                guard size == planLayer.expertStride,
                    offset + size <= planLayer.fileSize
                else {
                    throw RepackError.configurationInvalid(
                        detail:
                ...
                for slice in planLayer.subTensors {
                    ...
                    guard let subOffset = (sub["offset"] as? NSNumber)?.uint64Value,
                        let subSize = (sub["size"] as? NSNumber)?.uint64Value
                    else {
                        throw RepackError.configurationInvalid(
                            detail:
