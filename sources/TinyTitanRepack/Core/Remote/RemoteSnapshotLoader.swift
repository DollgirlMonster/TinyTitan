...
enum RemoteSnapshotLoader {
    static func load(
        ...
        if requireKnownSource
            && SourceFingerprint.modelID(forIndexSha256: metadata.indexSha256Hex) == nil
        {
        ...
        for shard in metadata.shardFilenames {
            ...
            files[shard] = info
            let prefix = try await pinned.downloadRangeToTempFile(
                filename: shard,
                info: info,
                offset: 0,
                length: 8,
            ...
            guard prefixData.count == 8 else {
                throw RepackError.safetensorsHeaderInvalid(
                    path: shard, detail: "short header prefix")
            }
            ...
            let headerFile = try await pinned.downloadRangeToTempFile(
                filename: shard,
                info: info,
                offset: 8,
