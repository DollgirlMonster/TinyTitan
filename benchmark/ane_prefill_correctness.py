...
def run_arm(model: str, arm: str, characters: int, chunk: int, max_new: int,
...
def summarize(model: str, arms: dict[str, dict], max_new: int) -> dict:
    out: dict = {"model": model, "max_new_tokens": max_new,
                 "measured": {name: {"decode_tokens": run.get("decode_tokens"),
    ...
    for name in ("on", "causal"):
        if name in arms and "error" not in arms[name] and not arms[name]["used_ane"]:
            out["error"] = (f"the {name} arm fell back to the GPU: "
                            f"{arms[name].get('fallback_reason')}")
            return out
    ...
    if on is not None and causal is not None:
        if not arms["causal"].get("causal_warning"):
            out["error"] = ("the causal control did not announce itself; it may "
                            "have run the folded path")
            return out
...
def main() -> int:
    ...
    results = []
    header = (f"{'model':<44} {'identical':>9} {'prefix':>9}  "
              f"causal divergence")
    ...
    for name in args.models:
        ...
        for arm in wanted:
            run = run_arm(name, arm, args.prompt_characters,
                          args.prefill_chunk, args.max_new)
            arms[arm] = run
            if "error" in run:
                break
        row = summarize(name, arms, args.max_new)
        row["arms"] = {arm: {k: v for k, v in run.items()
                             if k != "response"} for arm, run in arms.items()}
