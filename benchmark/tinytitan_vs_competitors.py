#!/usr/bin/env python3
"""Head-to-head decode throughput: TinyTitan against the other local-inference
engines on Apple silicon, on one machine, one model, one set of prompts.

    python3 benchmark/tinytitan_vs_competitors.py --check     # readiness only
    python3 benchmark/tinytitan_vs_competitors.py             # run everything
    python3 benchmark/tinytitan_vs_competitors.py --engines tinytitan,llamacpp
...
not a lowest-common-denominator config that happens to suit TinyTitan:

  * all layers on the GPU, flash attention on, where the engine has the knob
  * 8-bit KV cache everywhere, because that is TinyTitan's shipped default and
...
They are TinyTitan's own routing-diversity ladder, from code (diverse expert
routing, worst cache locality) to repeated digits (maximally repetitive, best
locality). For a sparse MoE the spread between them is large -- 27% on TinyTitan at
...
PROMPTS = [
    (
        "code",
        "Write a Python function that computes the Levenshtein distance "
    ...
    (
        "digits",
        "Write the digits 1,2,3,4,5,6,7,8,9,0 over and over in "
...
WARMUPS = 1
REPEATS = 2
CONTEXT = 8192  # every engine must hold this; TinyTitan's native is larger
KV_BITS = 8  # TinyTitan's shipped default -- matched everywhere possible
...
TINYTITAN_MODEL = MODELS_DIR / "ornith-1.5_35B_A3B_4Bit"
...
def run_llamacpp(prompt: str) -> tuple[float | None, float | None, str]:
    ...
    # -ctk/-ctv q8_0  match TinyTitan's 8-bit KV rather than leaving it at fp16
...
def run_mlx(prompt: str) -> tuple[float | None, float | None, str]:
    ...
    # --kv-bits 8 matches TinyTitan; without it MLX holds fp16 KV and looks worse
...
def run_tinytitan(prompt: str) -> tuple[float | None, float | None, str]:
    ...
    # "TinyTitan generation ... decode_tok_s=" footer, prefill excluded. Anything
    # else would be measuring TinyTitan differently from how we measure it
    ...
    try:
        ...
        while time.time() - start < 2400:
            if proc.poll() is not None:
                return None, None, "TinyTitanServer exited during load"
            try:
            ...
            return None, None, "TinyTitanServer never became ready"
        payload = json.dumps(
            {
                "model": model_id,
                "messages": [{"role": "user", "content": prompt}],
...
ENGINES: dict[str, tuple[Engine, object]] = {
    ...
    "ollama": (
        Engine(
            ...
            caveat="flash-attn and q8_0 KV are set via env on `ollama serve`, not per request",
        ),
        run_ollama,
    ),
    "mlx": (
    ...
    "mlc": (
        Engine(
            ...
            extra_check=lambda: (
                None if _has_module("mlc_llm") else "python module mlc_llm not importable"
            ),
        ),
        run_mlc,
    ),
...
def main() -> int:
    ...
    ap.add_argument("--engines", default=",".join(ENGINES), help="comma-separated subset")
    ...
    print(f"\n=== summary (decode tok/s, best of {args.repeats}; prefill excluded) ===")
